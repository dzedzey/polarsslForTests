...
#include "polarssl/memory.h"
    ...
    !defined(POLARSSL_THREADING_C) || !defined(POLARSSL_THREADING_WINTHREAD)
...
static void *handle_ssl_connection( void *data )
{
    ...
    sprintf( pers,"SSL Win Thread %d", thread_id );
...
static int thread_create( int client_fd )
{
	...
	if( ( threads[i].thread = CreateThread( NULL, 0, ( LPTHREAD_START_ROUTINE )handle_ssl_connection, &threads[i].data, 0, &ret  ) ) == 0 )
	{
		return( ret );
	}
